        ...
        appendArrayElement(this, indent, trailingComma) {
            ...
            forEachPhaseMeasurement { phaseType, time ->
                ...
                val key = when (phaseType) {
                    ...
                    PhaseMeasurementType.BackendGeneration -> "backendGenerationStats"
                }
            ...
            if (gcStats.isNotEmpty()) {
                appendArray("gcStats", gcStats, indent + 1, trailingComma = true) { gcStat ->
                    appendArrayElement(gcStat, indent + 2, trailingComma = false) {
                        appendKeyValue("kind", gcStat.kind, indent = 2)
                        appendKeyValue("count", gcStat.count, indent = 2)
                        appendKeyValue("milliseconds", gcStat.millis, indent = 2, trailingComma = false)
                    }
                }
            }
...
private fun StringBuilder.appendTime(key: String, time: Time, indent: Int, trailingComma: Boolean) {
    appendObject(key, time, indent, trailingComma) {
        ...
        appendKeyValue("cpuNane", time.cpuNano, indent + 1, trailingComma = false)
    }
}
...
private fun StringBuilder.appendIndent(indent: Int) {
    (0..indent).forEach { _ ->
        indentCache.add(" ".repeat(indent * 2))
    }
