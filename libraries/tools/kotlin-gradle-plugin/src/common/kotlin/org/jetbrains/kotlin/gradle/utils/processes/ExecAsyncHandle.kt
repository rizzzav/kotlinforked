...
sealed interface ExecAsyncHandle {
    val displayName: String

    /**
     * Start, without blocking.
     *
     * Use [waitForFinish] to get the result.
     */
...
private class ExecAsyncHandleImpl(
    ...
    //    private val result: AtomicReference<ExecResult> = AtomicReference()
    private var result: ExecResult? = null
...
//            result.set(run())
    ...
    override fun start(): ExecAsyncHandle {
        logger.info("[ExecHandle $displayName] starting... (${thread.state})")
        thread.start()
        return this
    }
    ...
    override fun waitForFinish(): ExecResult {
        ...
        try {
            ...
            logger.info("[ExecHandle $displayName] interrupted $ex (${thread.state})")
            thread.interrupt()
            thread.join()
        }

        return result ?: throw IllegalStateException("[ExecHandle $displayName] result is null (${thread.state})")
    }

    override fun abort() {
        logger.info("[ExecHandle $displayName] aborting... (${thread.state})")
        thread.interrupt()
        logger.info("[ExecHandle $displayName] finished aborting (${thread.state})")
    }
...
internal class ExecAsyncHandleImpl2(
    override val displayName: String,
    private val run: () -> ExecResult,
) : ExecAsyncHandle {
    /** Delay launching [result] until [trigger] has completed. */
    private val trigger: CompletableFuture<Unit> = CompletableFuture()

    private val result: CompletableFuture<ExecResult> = trigger.thenApplyAsync({ run() }, defaultExecHandleExecutor)

//    private val future: CompletableFuture<ExecResult> by lazy(mode = SYNCHRONIZED) {
//        CompletableFuture.supplyAsync(run, executor)
//            .exceptionally { throwable ->
//                throw ExecException("Execution failed for task '$displayName'.", throwable)
//            }
//    }

//    private val result: ExecResult by lazy {
//        future.get()
//    }

    override fun start(): ExecAsyncHandle {
        logger.info("[ExecHandle $displayName] starting...")

        // Start the future by completing the trigger
        trigger.complete(Unit)

        val result = result.get()
        logger.info("[ExecHandle $displayName] completed with exit value ${result.exitValue}")
    ...
    override fun waitForFinish(): ExecResult {
        try {
            ...
            logger.info("[ExecHandle $displayName] interrupting... $ex")
            trigger.cancel(true)
            result.cancel(true)
            trigger.join()
            result.join()
            logger.info("[ExecHandle $displayName] finished interrupting")
        }
        return result.get()
    }

    override fun abort() {
        logger.info("[ExecHandle $displayName] aborting...")
        ...
        logger.info("[ExecHandle $displayName] finished aborting")
    }

    override fun isAlive(): Boolean {
        return !result.isDone || !trigger.isDone
    }

    companion object {
        private val logger: Logger = Logging.getLogger(ExecAsyncHandleImpl::class.java)
         ...
         */
        private val defaultExecHandleExecutor: ExecutorService by lazy {
